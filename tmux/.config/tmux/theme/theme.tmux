...
set() {
	...
	local option=$2
	local value=$3
	tmux_commands+=(set-option "$flag" "$option" "$value" ";")
}
...
setw() {
	...
	local option=$2
	local value=$3
	tmux_commands+=(set-window-option "$flag" "$option" "$value" ";")
}
...
main() {
	...
	set -g status-left "\
		#{?client_prefix,#[fg=#{@theme_black}#,bg=#{@theme_red}],#[fg=#{@theme_black}#,bg=#{@theme_blue}]} #S \
		#{?client_prefix,#[fg=#{@theme_red}#,bg=#{@theme_black}],#[fg=#{@theme_blue}#,bg=#{@theme_black}]}"

	set -g status-right "\
		#[fg=#{@theme_gray},bg=#{@theme_black}]\
		#[fg=#{@theme_blue},bg=#{@theme_gray}] %d-%m-%Y  %H:%M \
		#[fg=#{@theme_blue},bg=#{@theme_gray}]\
		#[fg=#{@theme_black},bg=#{@theme_blue},bold] #h "
	...
	setw -g window-status-current-format "\
		#[fg=#{@theme_black},bg=#{@theme_gray}]\
		#[fg=#{@theme_blue},bg=#{@theme_gray},bold] #I  #W #F \
		#[fg=#{@theme_gray},bg=#{@theme_black}]"
